...
interface MemberCurationShowcaseProps {
  onMemberChange?: (member: Member) => void;
}

export const MemberCurationShowcase: React.FC<MemberCurationShowcaseProps> = ({ onMemberChange }) => {
  ...
  // Notify parent on member change if needed
  useEffect(() => {
    if (onMemberChange) {
      onMemberChange(currentMember);
    }
  }, [currentMember, onMemberChange]);

  // 3 featured merchandise items specifically for the active member
  ...
  return (
    <div
      className="bg-white dark:bg-black text-black dark:text-white select-none transition-colors"
    ...
    >
      {/* Main Editorial Showcase Grid */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
        {/* Left Column: Editorial Headline, Bio, Quote, CTA & Featured Products */}
        <div className="lg:col-span-6 space-y-6 order-2 lg:order-1">
          <div className="space-y-3">
            <span className="text-[11px] font-medium tracking-editorial uppercase text-[#777777] dark:text-[#888888] block">
              EDITORIAL CURATION · 0{currentMember.order}
            </span>

            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-medium tracking-tight uppercase leading-[1.1]">
          ...
          <div className="pt-1">
            <Link
              ...
              className="inline-flex items-center gap-2 px-6 py-3 bg-black text-white dark:bg-white dark:text-black text-xs font-medium tracking-editorial uppercase rounded-[2px] hover:opacity-85 transition-opacity"
            >
          ...
          {/* Featured Related Merchandise Previews */}
              ...
              {memberProducts.map((product, idx) => (
                ...
                >
                  <Link
                    ...
                    className="group block bg-[#F7F7F7] dark:bg-[#111111] border border-[#E5E5E5] dark:border-[#292929] rounded-[2px] p-2 hover:border-black dark:hover:border-white transition-colors"
                  >
        ...
        {/* Right Column: Square 1:1 Member Image with Arrows Flanking the Image ([ ← ] [ IMAGE ] [ → ]) */}
        <div className="lg:col-span-6 flex items-center justify-center lg:justify-end gap-2 sm:gap-3.5 order-1 lg:order-2">
          {/* Left Arrow */}
          <button
            type="button"
            onClick={onPrevClick}
